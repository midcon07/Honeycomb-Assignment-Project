...
- `tools/Preflight/` — **the gate that runs on every launch**, not only during setup. Returns `GO`, `NO-GO`, `SETUP NEEDED` or `CANNOT RUN`, and the program must not start the simulator on `NO-GO`. Checks are pluggable: adding an add-on means dropping a file in `Preflight/Checks/`, never editing the host. FSUIPC is the first check — installation, licence, device scan and settings. Read-only, verified byte-identical across a run; `-Json` writes the machine report. Needs `-ExecutionPolicy Bypass`, as every scope is Undefined on a stock Windows install. See `tools/Preflight/README.md` for the contract.
- `tools/Get-SimBriefPlan.ps1` — step two of the launch flow: fetches the latest SimBrief plan, judges whether it is recent enough to be today's flight, and resolves the aircraft to a lever layout. Advisory only and can never block a flight — every failure, including no internet, produces a plain sentence and the flow carries on. Field names were verified against a real response rather than taken from documentation.
