...
- `tools/Check-FSUIPCHealth.ps1` — read-only health check of FSUIPC7 and its relationship to MSFS 2024, and the first piece of the health-check engine in `docs/setup-spec.md`. Discovers FSUIPC and the sim rather than assuming paths, handles both Store and Steam installs, reads the Community folder from `InstalledPackagesPath`, and names any inactive Community folders it finds. Distinguishes *not yet configured* from *misconfigured*, so a fresh machine does not look broken. `-Json` writes the machine report. Needs `-ExecutionPolicy Bypass`, as every scope is Undefined on a stock Windows install.
