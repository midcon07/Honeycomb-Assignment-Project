...
- **GOAL (Mark, 2026-09-06): the same system for the Honeycomb Alpha.** Plan and decisions in `notes/alpha-plan.md`. First step done: `data/alpha-buttons.json` (34 controls; 11 yoke buttons/rockers measured by Mark 2026-09-07 = panel 1-12 less the left horn's left rocker UP) and the capture follows the table's `device`. Next: Mark captures the remaining 23 (hat, base switches, key); then decisions 1-4 in the plan; then the writer is generalised with fenced per-device blocks.
