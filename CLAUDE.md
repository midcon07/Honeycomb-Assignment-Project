...
- **MSFS 2024 settings on disk, measured 2026-09-07 with the sim running:** `UserCfg.opt` is rewritten within a second or two of EVERY settings change (not only at exit); its `{Graphics}{Traffic}` block holds the density/variety settings as LEVELS: **-1 = Off, 3 = Ultra (measured 2026-09-12, Mark at the page, file diffed live); 0-2 not read back against their words** (the earlier reading of -1 as "never set" was wrong). BATC/FSLTL need Aircraft Traffic and Parked Aircraft Off; MSFS Ultra. These two ARE read by the program (`SimSettings.cs`, preflight `37-TrafficGraphics.ps1`, the sheet's part 1 confirms itself; `HONEYCOMB_USERCFG` points tests at a copy). The Options > General > Traffic page (AI traffic type, ground aircraft, multiplayer) is NOT in any readable file: it is in the cloud-synced profile container under `SystemAppData\wgs` (`profile_00` in the index, ~1.18 MB, a new blob per save, also written seconds after a change, plain bytes with readable strings). **Measured 2026-09-11: Traffic Type (Options > General > ONLINE in 2024) is not in that container either - three saves byte-compared, the only differences are a save counter, a 108 KB memory-dump block and a checksum, identical in kind between two saves of the SAME state. It lives only in Microsoft's cloud profile. Do not chase it again; it is a person's word.** The container does hold, in clear, the aircraft/EFB datastore and the Navigraph and SimBrief sign-in tokens: never copy it anywhere. Three traffic modes now (BATC / FSLTL = Traffic Type Off; MSFS = Real-Time Online): `notes/traffic-plan.md`.
