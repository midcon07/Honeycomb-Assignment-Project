...
- **The tractor-feed reminder sheet is the pattern for anything a person must do inside the sim** (`TrafficReminderForm` + `DotMatrix`): always on top, prints the instruction character by character with the printer's sounds, a clickable DONE that strikes through and records who/when as THEIR WORD, a NOT NOW that comes back next occasion; opens pinned and open and never collapses by itself; an anchor lamp top-left (green = on top, red = ordinary window; click to toggle; dropping after a drag anchors it); printed chrome top-right: A- A+ (pitch saved as `printoutPitch`), minimise to a small paper square (`PrintoutIconForm`) that restores the same sheet, maximise, close; resizable by its edges with the text reflowing; draggable to any monitor. Mark has more plans for this window - it is the program's printout console, not a one-off dialog: **more information will be pushed through it during the flight** (Mark, 2026-09-11), so build it as an append-only feed with printed, clickable lines. Closing it asks first, printed on the sheet (YES / NO); the program replaces a sheet with `CloseWithoutAsking`. It remembers its place (`printoutBounds`) and the launcher remembers its own (`launcherBounds`) - the launcher used to open centred under the full-screen sim. `--traffic-sheet` shows it standalone. Sounds are synthesised (no media files) and written once to `%LOCALAPPDATA%\HoneycombAssignment\sounds`; `System.Media.SoundPlayer` from a file path, never from a MemoryStream (that crashed PowerShell with an access violation on 2026-09-07).
- **PowerShell variable names are case-insensitive, and a typed parameter is a typed variable (2026-09-10, two bugs in one function).** `[string] $Section` as a parameter plus `$section = New-Object ArrayList` as a local: the ArrayList was converted to a string on assignment and `.Add` failed. `$J = 'B'` then `$j = Get-...` in the same function: one variable, so every line compared equal to itself and the user's lines were all classified as ours. Never reuse a parameter's name in any case for a local; name locals differently (`$secLines`, `$ownedJoy`, `$lineJoy`).
- **The writers own a PART of a file, never the file (audit findings 1 and 2, fixed 2026-09-10 in `tools/FsuipcIni.ps1`).** Global `[Buttons]`: every numbered line whose button is on the quadrant's joystick letter is ours to replace (parsed from the line: `P/U/R<j>,<b>` and `CP(+<j>,<c>)<j>,<b>`, letter or number); lines for any other device are kept in their order, renumbered from 0, ahead of ours; `PollInterval` is forced to 10, `ButtonRepeat` kept as the user has it; other keys and comments kept at the top; the writer names any replaced line it did not write itself. `myevents.txt`: our presets live between two `// ----` marker lines; the user's lines outside are untouched; an older plain copy of our file (every machine set up before 2026-09-10, Mark's included) is fenced on the next write (`converted`), stray identical lines of ours outside the block are absorbed, and a same-named preset with different code refuses the whole presets install and says so. `tools/Test-FsuipcIniMerge.ps1` (50 checks) covers all of it and runs `Set-BravoButtons.ps1` for real against a scratch copy of this machine's ini with a user's lines added; run it after touching either writer.
...
- **TOMORROW (parked 2026-09-07 23:50, Mark: "Save this for tomorrow"):**
  ...
  2. **AI traffic** - mode choice and the tractor-feed reminder sheet are BUILT (2026-09-11, `notes/traffic-plan.md`); Mark liked the print sound, the line feed was redone (the first one "sounded like a snort"), print speed +10%. Next: the flight-time confirmation by counting non-injected AI aircraft, and preflight rows per mode. **Mark's taste for this program: the 1990s/early-2000s airline era - dot-matrix printouts, tractor-feed paper, the sounds to match. Keep new surfaces in that world.**
  ...
  4. **Lead, downgraded 2026-09-11: the profile container's readable part is the aircraft/EFB datastore, not the sim's own options** (Traffic Type left no trace). Which input profile is assigned to which controller MAY still be in there (GUIDs and "Midcon General" are); untested, and the sim's own options were not, so expect the same result.
...
- **Security audit 2026-09-05 → `notes/security-audit-2026-09-05.md`**, ten items as a checklist. **1, 2 and 4 done 2026-09-10** (`tools/FsuipcIni.ps1`; fonts bundled in `ui/fonts`). The page makes no outbound request of its own now - keep it that way: any new external resource goes through the host, never a page fetch.
...
- **MSFS 2024 settings on disk, measured 2026-09-07 with the sim running:** `UserCfg.opt` is rewritten within a second or two of EVERY settings change (not only at exit); its `{Graphics}{Traffic}` block holds the density/variety settings as LEVELS: **-1 = Off, 3 = Ultra (measured 2026-09-12, Mark at the page, file diffed live); 0-2 not read back against their words** (the earlier reading of -1 as "never set" was wrong). BATC/FSLTL need Aircraft Traffic and Parked Aircraft Off; MSFS Ultra. These two ARE read by the program (`SimSettings.cs`, preflight `37-TrafficGraphics.ps1`, the sheet's part 1 confirms itself; `HONEYCOMB_USERCFG` points tests at a copy). **2026-09-12 evening, from Mark's session with midcon07:** the sheet has three parts (graphics read from the file; Traffic Type as a person's word; the traffic engine - `TrafficEngines.cs` finds BeyondATC / the FSLTL injector, the sheet offers `[ ] Start <engine> now` once the graphics are right, the process watcher confirms it in print); printer options on the sheet's right-click menu (ink weight, both directions, mixed case, speed; config `printout*`); the launcher watches the sim's process (button says Simulator Running / Start Simulator; a sheet prints when the sim comes up). The LaserWriter (Printer / Face on the same menu; proportional faces, the page reprinted whole with the CX-engine sound sequence) was built the same evening. **Standing direction: "80s - 90s vibe is our ultimate visual UI goal."** The launcher's page became a 1989 VGA text-mode terminal on a CRT (style block replaced whole, DOM untouched); **both faces are in the page now - "[ Modern ]" / "[ 1989 ]" in the bar toggles them, config `uiEra`, default 80s;** the printer got an airport under it (`Ambience.cs`: faint jets, an occasional turboprop, PA announcements by Windows' own voice through a horn; NAudio + System.Speech). Details: `notes/traffic-plan.md`. Item 0 (Global Rendering Quality reads Custom - leave it) is an assumption to confirm. Details: `notes/traffic-plan.md`. The Options > General > Traffic page (AI traffic type, ground aircraft, multiplayer) is NOT in any readable file: it is in the cloud-synced profile container under `SystemAppData\wgs` (`profile_00` in the index, ~1.18 MB, a new blob per save, also written seconds after a change, plain bytes with readable strings). **Measured 2026-09-11: Traffic Type (Options > General > ONLINE in 2024) is not in that container either - three saves byte-compared, the only differences are a save counter, a 108 KB memory-dump block and a checksum, identical in kind between two saves of the SAME state. It lives only in Microsoft's cloud profile. Do not chase it again; it is a person's word.** The container does hold, in clear, the aircraft/EFB datastore and the Navigraph and SimBrief sign-in tokens: never copy it anywhere. Three traffic modes now (BATC / FSLTL = Traffic Type Off; MSFS = Real-Time Online): `notes/traffic-plan.md`.
