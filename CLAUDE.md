...
- **The tractor-feed reminder sheet is the pattern for anything a person must do inside the sim** (`TrafficReminderForm` + `DotMatrix`): always on top, prints the instruction character by character with the printer's sounds, a clickable DONE that strikes through and records who/when as THEIR WORD, a NOT NOW that comes back next occasion; a pushpin keeps it on top, otherwise it collapses to an icon (`PrintoutIconForm`) and expands back with content intact; draggable to any monitor. Mark has more plans for this window - it is the program's printout console, not a one-off dialog. `--traffic-sheet` shows it standalone. Sounds are synthesised (no media files) and written once to `%LOCALAPPDATA%\HoneycombAssignment\sounds`; `System.Media.SoundPlayer` from a file path, never from a MemoryStream (that crashed PowerShell with an access violation on 2026-09-07).
